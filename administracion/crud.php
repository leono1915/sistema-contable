...
date_default_timezone_set('America/Mexico_City');
 ...
 $rango=$_POST['rango'];
          ...
          switch($accion){
            ...
            case 'listarFiltrado': listarFiltrado(); break;
  ...
  /*---------------------------------------------- AQUI EMPIEZAN LAS FUNCIONES DEL SWITCH CASE -----------------------------------*/
   ...
   function  listarCotizaciones(){
     ...
     group by folio order by numero DESC limit $limitInf,$rango;");
          if(!$sql){
            die( 'error');
          } 
          $jason= array();
     ...
     and folio ='$folio' group by folio;");
         ...
         function listarFiltrado(){
          include '../conecta.php';
         $nombre=trim($_POST['nombre']);
         $mes=trim($_POST['mes']);
         $anio=trim($_POST['anio']);
         $facturado=trim($_POST['facturado']);
         $estatus=trim($_POST['estatus']);
         $mesConsulta='';
         $facturaConsulta='';
         if(strcmp($mes,'0')!==0){
            $mesConsulta="and MONTH(fecha)='$mes'";
         }
         if(strcmp($facturado,'facturado')!==0){
          $facturaConsulta="and historialventas.facturado='$facturado'";
       }
          //aqui hago mis uniones de la base de datos
          $consulta="select clientes.nombre,clientes.nombre_agente,usuarios.nombre as nombreU, historialventas.* from historialventas 
          join clientes join usuarios where (clientes.nombre LIKE '%$nombre%' or clientes.nombre_agente LIKE '%$nombre%'  ) and usuarios.id=historialventas.id_usuario 
          and clientes.id=historialventas.id_cliente ".$mesConsulta." ".$facturaConsulta. " and YEAR(fecha) ='$anio' and historialventas.estatus='$estatus' group by folio;";
     $sql=$dbConexion->query($consulta);
          if(!$sql){
            die( 'error');
          } 
          if($sql->num_rows==0){
...
,SUM(IF(YEAR(Fecha)=$anio&&facturado='no'&&estatus='autorizado'&&credito='no' ,Total,0)) As 'no_facturado'
 FROM  (select * from historialventas group by folio) as nu where YEAR(Fecha)=$anio group by mes) as ventas;
  ");
...
$sql=$dbConexion->query("
SELECT Mes,transferencia,efectivo,tarjeta,transferencia+efectivo+tarjeta as total, dia, efe
FROM (SELECT MONTH(Fecha) AS Mes,
 SUM(IF(Fecha='$dia'&&estatus='autorizado'&&credito='no',Total,0)) As 'dia'
,SUM(IF(Fecha='$dia'&&estatus='autorizado'&&credito='no'&&pago='efectivo',Total,0)) As 'efe'
  ...
  function  buscarClientes(){
    ...
    //                                         funcion eliminar clientes
         function  eliminarCliente(){
          ...
          if(!$sql){
            echo 'no se elimin車 el registro';
            die();
          } 

...
<head>
	...
	<script>
	var folio,archivo,cadena;
function eliminarTicket(objectpressed){
	var object=objectpressed.parentNode.parentNode;
		 folio=object.getElementsByTagName('td')[3].getElementsByTagName('p')[0].innerHTML;
		 cadena='cotizaciones/'
		 archivo=cadena+object.getElementsByTagName('td')[6].innerHTML;

		if(!confirm(`Está seguro de eliminar esta cotización?`)){
			
          return;
		}
		document.getElementById('ventana-emer').style.display='block';
		
}
	function elim(){
		var contra=document.getElementById('passs').value;
		if(contra!=='1671'){
			alert('contraseña incorrecta');
			document.getElementById('ventana-emer').style.display='none';
            return;
		}
		var respuesta='eliminarTicket';
...
<body onload="ocultar();">
	<!-- Page Wrapper -->
	<div id='ventana-emer' style='width:500px%; background-color:white;heigth:50hv; margin-top:19%; margin-left:30%; position:fixed'>
     <label for='passs' style='color:black'> 
	  Contraseña
	  <br>
	  
	 <input type="password" id='passs' style='width:500px;'></label>
	 <button onclick='elim();' class='principal' > Aceptar </button>
	 <button onclick='document.getElementById("ventana-emer").style.display="none"; ' class='principal' > cancelar </button>
	</div>
	<div id="page-wrapper">
...
<input id="buscarCliente" type="button" value="Filtrar" >
...
<input type="text" name="" placeholder="Nombre cliente" id="txtClienteNombre">
...
<select id="seleccion_anio"> 
      <option> <script> document.write(new Date().getFullYear()) </script></option>
	  <option> <script> var f=new Date().getFullYear(); document.write(parseInt(f)-1) </script></option>
	  <option> <script> var f=new Date().getFullYear(); document.write(parseInt(f)-2)</script></option>
	  <option> <script> var f=new Date().getFullYear(); document.write(parseInt(f)-3) </script></option>
...
<select id="seleccion_autorizado" onclick=" lis(); "> 
      <option> Autorizado</option>
	  <option> Pendiente</script></option>
...
<div class=" 2u 12u$(xsmall)" id="uuu">
   
   <script>
   document.getElementById('uuu').innerHTML= ` <select id="seleccion_facturado"> 
    ...
    function lis(){
		var pagoX=  document.getElementById('seleccion_autorizado').selectedIndex;
    var pagoY = document.getElementById('seleccion_autorizado').options;
    ...
    if(pago==='autorizado'){
	  ...
	  <option> NO</option>
		...
		document.getElementById('uuu').innerHTML='';
	} 
	}
								...
								<br><br>
								<table id="tabla_factura" style='width:920px'>
									<thead>
										<tr>
